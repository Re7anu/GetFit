...
def evaluate_pose_frame(req: Any) -> Any:
    """Evaluates joint angles, movement depth thresholds, and 2-phase rep state machine on backend."""
    import time
    from app.schemas.workout_log import FormFeedbackResponse, PoseFrameEvaluationResponse
    ...
    if exercise == "squats":
        ...
        plank_angle = 180
        if plank_visible:
            plank_angle = calculate_joint_angle_3d(lm[11], lm[23], lm[27]) if is_visible(11, 23, 27) else calculate_joint_angle_3d(lm[12], lm[24], lm[28])
